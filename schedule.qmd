...
Each project needs an end date and an estimate of remaining days. Optionally, you can say how many years of funding a project has left (`years_left`, counting the current year). This is useful for grants that run for several more years: the planner creates one follow-on year per remaining year, each shrinking by an assumed annual salary raise, since a grant year is a fixed pot of money. Set `renewal_days` to pin the first follow-on year's budget instead of deriving it.
...
from planner.workers import DEFAULT_WORKER, worker_config_path
from planner.analysis import (
    load_projects,
    filter_projects_by_probability,
...
# Configuration
# Render another worker's schedule with: PLANNER_WORKER=alice quarto render schedule.qmd
WORKER = os.environ.get("PLANNER_WORKER", DEFAULT_WORKER)
CONFIG_FILE = str(worker_config_path(WORKER))
NUM_WEEKS = 104
LAST_UPDATED = date(2026, 5, 4)  # Date when the worker's projects file was last updated
MIN_PROBABILITY = 0.5  # Minimum probability threshold for scenario analysis

# Set date to when data was updated
start_date = LAST_UPDATED

# Load all projects and filter to active only (probability = 1.0)
all_projects = load_projects(CONFIG_FILE)
projects = filter_projects_by_probability(all_projects, 1.0)

# Create schedules with active projects only
...
This section shows schedules for active projects only (probability = 1.0), excluding pending proposals.
...
```{python}
#| label: fig-project-allocation
#| fig-cap: "Weekly project allocation: percentage of time spent on each project (smoothed over 4 weeks)"
#| column: page

fig_allocation = create_project_allocation_plot(schedule_paced, NUM_WEEKS)
if fig_allocation:
    fig_allocation.show()
```

# Scenario: Including Pending Projects

This section shows the schedule when including projects with probability above a confidence threshold (set via MIN_PROBABILITY). This helps visualize workload when pending proposals are likely to be funded.
...
# Filter projects by probability threshold
high_confidence_projects = filter_projects_by_probability(all_projects, MIN_PROBABILITY)

print(f"Total projects (including pending): {len(all_projects)}")
print(f"Active projects (probability = 1.0): {len(projects)}")
print(f"High-confidence projects (probability >= {MIN_PROBABILITY}): {len(high_confidence_projects)}")
print(f"Additional projects in this scenario: {len(high_confidence_projects) - len(projects)}")

if len(high_confidence_projects) > len(projects):
    print("\nAdditional projects included:")
    for p in all_projects:
        if p.probability >= MIN_PROBABILITY and p.probability < 1.0:
            print(f"  - {p.name} (probability: {p.probability})")
```
...
# Create schedules with filtered projects
scheduler_filtered = Scheduler(high_confidence_projects, start_date=LAST_UPDATED)
schedule_filtered = scheduler_filtered.create_schedule(num_weeks=NUM_WEEKS, method="paced")

# Compute availability
filtered_availability = compute_weekly_availability(schedule_filtered, NUM_WEEKS)
```

## Paced Method (With Pending Projects)

```{python}
#| label: fig-filtered-calendar
#| fig-cap: "Paced scheduling including pending projects with probability >= threshold"
#| column: page

fig_filtered_cal = create_calendar_heatmap(schedule_filtered, f"Including Pending Projects (probability >= {MIN_PROBABILITY})")
if fig_filtered_cal:
    fig_filtered_cal.show()
```

```{python}
#| echo: false
#| label: tbl-filtered-unassigned
#| tbl-cap: "Unassigned days per month (Including Pending Projects)"

unassigned_filtered = compute_monthly_unassigned_days(schedule_filtered, include_zero_months=True)
unassigned_filtered["cumulative_unassigned_days"] = unassigned_filtered["unassigned_days"].cumsum()
if not unassigned_filtered.empty:
    display(unassigned_filtered[['month_name', 'unassigned_days', 'cumulative_unassigned_days']].rename(
        columns={'month_name': 'Month', 'unassigned_days': 'Unassigned Days', 'cumulative_unassigned_days': 'Cumulative Unassigned Days'}
    ))
```

## Project Allocation (With Pending Projects)

```{python}
#| label: fig-filtered-allocation
#| fig-cap: "Weekly project allocation including pending projects (smoothed over 4 weeks)"
#| column: page

fig_filtered_allocation = create_project_allocation_plot(schedule_filtered, NUM_WEEKS)
if fig_filtered_allocation:
    fig_filtered_allocation.show()
```
